...
def _question_lines(question: str, max_lines: int = 6) -> list[str]:
    ...
    if not lines:
        return ["（见源稿题目原文）"]
    return [_trim(ln, 140) for ln in lines[:max_lines]]
...
def _compare_table_rows() -> list[list[str]]:
    return [
        ["内容", "任务齐全", "细节 + 主题", "逻辑链 + 对比"],
        ["逻辑", "简单并列", "情感过渡", "Firstly / Unlike"],
        ["词汇", "good / nice", "resonated / vulnerability", "symbolizes / cohesive"],
        ["句式", "简单句", "从句 + 经历", "因果 + 比较"],
    ]
...
def _stage4_migration(stage4: str) -> list[str]:
    from scripts.classroom_content_filter import parse_stage4_student_from_export

    pts = parse_stage4_student_from_export(stage4)["migration"]
    return pts if pts else [
        "片段升级：换主题，保持 PEEL 结构",
        "完整迁移：限时写主体段",
    ]
...
def build_slot_spec(slot: PageSlot, data: dict[str, Any]) -> dict | None:
    ...
    if slot.slot_id == "D10":
        return {
            "type": "content",
            "title": slot.title,
            "bullets": ["画面 → 内涵 → 价值", *_bullets(stage2, 2)],
            "_module": slot.module,
        }
    if slot.slot_id == "F2s":
        return {
    ...
    if slot.slot_id == "F3s":
        return {
            ...
            "title": slot.title,
            "badge": "迁移练",
            "bullets": _stage4_migration(stage4),
            "_module": slot.module,
        }
    if slot.slot_id == "G1":
